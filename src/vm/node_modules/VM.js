    ...
    zoneadm(args, function (err, fds) {
        ...
        for (line in lines) {
            ...
            if (fields.length === 8 && fields[1] !== 'global') {
                ...
                VM.log.trace('loaded: ' + JSON.stringify(obj));
    ...
    getVmobj(info.zonename, data, function (err, vmobj) {
        ...
        loadMetadata(vmobj, function (error, metadata, tags) {
            ...
            for (snap in raw_snapshots) {
                ...
                if (!friendly_name) {
                    VM.log.warn('bad snapshot: ' + snap.name);
                    continue;
                }
                ...
                if (matches && matches[1]) {
                    ...
                    VM.log.debug('ignoring unfriendly ' + snap.name);
                    continue;
                }
            }
...
function writeZoneNetfiles(payload, callback)
{
    ...
    if (payload.hasOwnProperty('dns_domain')) hostname += '.' + payload.dns_domain;

    fs.writeFileSync(payload.zonepath + '/root/etc/nodename', hostname + '\n');


    // Sort out /etc/hosts file

    if (fs.existsSync(payload.zonepath + '/root/etc/hosts')) {
        var data = fs.readFileSync(payload.zonepath +
            '/root/etc/hosts').toString().replace(/\n$/, '').split("\n");
        fs.writeFileSync(payload.zonepath + '/root/etc/hosts', '');
        for (i in data) {
            if (!data[i].match('^::1(\s|\t)|^127.0.0.1(\s|\t)')) {
                fs.appendFileSync(payload.zonepath + '/root/etc/hosts',
                    data[i] + '\n');
            }
        }
    }
    ...
    if (payload.hasOwnProperty('resolvers')) {
        ...
        if (fs.existsSync(payload.zonepath + '/root/etc/resolv.conf')) {
            var data = fs.readFileSync(payload.zonepath +
                '/root/etc/resolv.conf').toString().replace(/\n$/, '').split("\n");

            fs.writeFileSync(payload.zonepath + '/root/etc/resolv.conf', '');

            var matchstr = '^nameserver';
            // if (payload.hasOwnProperty('dns_domain')) matchstr += '|^search';

            for (i in data) {
                if (!data[i].match(matchstr)) {
                    fs.appendFileSync(payload.zonepath + '/root/etc/resolv.conf',
                        data[i] + '\n');
                }
            }
        }
        ...
        //     fs.appendFileSync(payload.zonepath + '/root/etc/resolv.conf',
        //         'search ' + payload.dns_domain + '\n');
        // }

        for (i in payload.resolvers) {
            fs.appendFileSync(payload.zonepath + '/root/etc/resolv.conf',
                'nameserver ' + payload.resolvers[i] + '\n');
        }
    }
...
exports.waitForZoneState = function (payload, state, options, callback) {
    ...
    function handler(obj) {
        VM.log.trace('handler got: ' + JSON.stringify(obj));
    ...
    VM.load(payload.uuid, function (err, obj) {
        if (err) {
            ...
            VM.log.info('VM is in state ' + state);
...
exports.waitForProvisioning = function (vmobj, cb)
{
    ...
    function done() {
        ...
        if (ival_h) {
            VM.log.debug('closing hwsetup check interval for ' + vmobj.uuid);
    ...
    if (BRAND_OPTIONS[vmobj.brand].features.wait_for_hwsetup) {
        ival_h = markProvisionedWhenHWSetup(vmobj, {}, function (err) {
            if (err) {
    ...
    async.series([
        function (cb) {
            ...
            if (prov_wait) {
                ...
                cancel = VM.waitForProvisioning(vmobj, function (err) {
            ...
            VM.start(payload.uuid, {}, function (err, res) {
                if (err) {
                    ...
                    if (cancel) {
                        VM.log.info('cancelling VM.waitForProvisioning');
                        cancel();
                    }
                    cb(err);
                    return;
...
function startZone(zoneobj, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            // ensure it booted
            VM.waitForZoneState(zoneobj, 'running', {timeout: 30},
                function (err, result) {
                ...
                if (err) {
                    ...
                    // zone got to running
                    VM.log.info('VM seems to have switched to "running"');
                }
                cb(err);
            });
        }, function (cb) {
            zonecfg(['-u', uuid, set_autoboot], function (err, autoboot_fds) {
                if (err) {
                    // The vm is running at this point, erroring out here would
                    // do no good, so we just log it.
                    VM.log.error(err, 'startZone(): Failed to ' + set_autoboot
            ...
            zonecfg(['-u', uuid, 'remove attr name=never-booted' ],
                function (err, neverbooted_fds) {
                    ...
                    if (err) {
                        VM.log.warn(err, 'failed to remove never-booted '
...
function startVM(vmobj, extra, callback)
{
    ...
    cmdargs.push('-smbios', 'type=1,manufacturer=EveryCity,'
        + 'product=EveryCity HVM,version=6.2012Q1,'
...
//
function validSnapshotName(snapname)
{
    if (snapname.length < 1 || snapname > MAX_SNAPNAME_LENGTH) {
        VM.log.error('Invalid snapname length: ' + snapname.length
            + ' valid range: [1-' + MAX_SNAPNAME_LENGTH + ']');
        return (false);
    }

    if (snapname.match(/[^a-zA-Z0-9\-\_\.\:\%]/)) {
        VM.log.error('Invalid snapshot name: contains invalid characters.');
        return (false);
    }
...
function performSnapshotRollback(snapshots, callback)
{
    ...
    function rollback(snapname, cb) {
        ...
        zfs(args, function (zfs_err, fds) {
            if (zfs_err) {
                VM.log.error({'err': zfs_err, 'stdout': fds.stdout,
                    'stderr': fds.stdout}, 'zfs rollback of ' + snapname
                    + ' failed.');
                cb(zfs_err);
                return;
            }
            VM.log.info('rolled back snapshot ' + snapname);
            VM.log.debug('zfs destroy stdout: ' + fds.stdout);
            VM.log.debug('zfs destroy stderr: ' + fds.stderr);
            cb();
        });
    }
    ...
    async.forEachSeries(snapshots, rollback, function (err) {
        if (err) {
            VM.log.error(err, 'Unable to rollback some datasets.');
        }
        callback(err);
    });
}
...
{
    ensureLogging('rollback_snapshot', true);
    ...
    VM.load(uuid, function (err, obj) {
        ...
        if (obj.state !== 'stopped') {
            VM.stop(obj.uuid, {'force': true}, function (stop_err) {
                if (stop_err) {
                    VM.log.error(stop_err, 'failed to stop VM ' + obj.uuid
                        + ': ' + stop_err.message);
                    callback(stop_err);
                    return;
                }
                performSnapshotRollback(snapshot_list, function (rollback_err) {
                    if (rollback_err) {
                        VM.log.error(rollback_err, 'failed to '
...
{
    ensureLogging('delete_snapshot', true);
...
{
    ensureLogging('create_snapshot', true);
    ...
    VM.load(uuid, function (err, obj) {
        ...
        if (obj.hasOwnProperty('snapshots')) {
            for (snap in obj.snapshots) {
                ...
                if (snap.name === full_snapname) {
                    ...
                    VM.log.debug('SKIPPING ' + snap.name);
                }
            }
        }
        ...
        VM.log.info('Taking snapshot "' + snapname + '" of ' + uuid);
        ...
        zfs(args, function (zfs_err, fds) {
            ...
            VM.log.debug('zfs snapshot stdout: ' + fds.stdout);
            VM.log.debug('zfs snapshot stderr: ' + fds.stderr);
...
function setRctl(zonename, brand, rctl, value, callback)
{
    var cmd = '/usr/bin/prctl -n ' + rctl + ' -v ' +  value.toString() + ' -r -i ';
    if (brand == 'solaris10') {
        cmd += 'process `/usr/bin/pgrep -oxz ' + zonename + ' init`';
    } else {
        cmd += 'zone ' + zonename;
    }
    VM.log.debug(cmd);
    ...
    async.series([
        function (cb) {
            ...
            async.forEachSeries(keys, function (prop, c) {
                ...
                if (payload.hasOwnProperty(prop)) {
                    setRctl(newobj.zonename, newobj.brand, rctl,
