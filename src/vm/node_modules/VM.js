...
function lookupConflicts(macs, ips, vrids, log, callback) {
    ...
    vmload.getVmobjs(function (vm, cb) {
        ...
        for (ip in ips) {
            if (ips[ip] !== 'dhcp' && matcher(vm, {'nics.*.ip': ips[ip]})) {
...
function createHostConfFileMounts(vmobj, log, callback) {
    ...
    if (vmobj.nics && vmobj.nics.length > 0 && vmobj.nics[0].ip) {
        hosts.unshift([vmobj.nics[0].ip, hostname]);
    }
        ...
        fs.lstat(f, function (error, stats) {
            ...
            if (error) {
                if (error.code === 'ENOENT') {
                    log.info('creating empty file for mountpoint: ' + f);
                    dir = path.dirname(f);
                    mkdirp(dir, function (err) {
                        ...
                        try {
                            ...
                            log.error({err: e}, 'failed to create ' + f);
                            cb(e);
                            return;
                        }
...
function startVM(vmobj, extra, log, callback)
{
    ...
    cmdargs.push('-smbios', 'type=1,manufacturer=EveryCity,'
        + 'product=EveryCity HVM,version=7.' + smbios_version + ','
...
function doDockerStop(vmobj, options, callback)
{
    ...
    // if init has died but zone is still running, pid is reported by mdb to be
    // 4294967295 (UINT32_MAX) so if we see that value, just halt the zone.
    ...
    if (err && err.code === 'ESRCH') {
        ...
        /*
         * kill(2) says this should be EPERM or EINVAL which are both programmer
         * errors here.
         */
        callback(err);
        return;
    ...
    /*
     * We expect the stop to have completed within options.timeout seconds, if
     * it hasn't we'll try SIGKILL and if that fails to kill the zone, we'll
     * do a 'zoneadm halt'.
     */
    timer = setTimeout(function () {
        /*
         * Hit timeout, do kill -KILL, ignore error since either process is gone
         * and doPwait will notice and exit, or we'll timeout a second time.
         */
         ...
         * doesn't though set one more timer. If that expires, halt the zone.
         */
        timer = setTimeout(function () {
            if (waiter) {
                waiter.kill();
