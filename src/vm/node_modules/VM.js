...
function buildNicZonecfg(vmobj, payload, log)
{
    ...
    for (nic in add) {
        ...
        if (nic.hasOwnProperty('gateways') && nic.gateways.length > 0) {
            repr_ip = ipaddr.getRepresentativeIP(nic.gateways, 'ipv4-strict');
        ...
        if (nic.hasOwnProperty('ips') && nic.ips.length > 0) {
 ...
 *
 *   - tty and any other logdriver == 'none'
 *       - don't write to the log in the GZ, zlog-mode=nlinteractive
 *
 ...
 *       - set attr app-svc-dependent = true
 *
 *   - no tty and any other logdriver == 'none'
 *       - don't write to the log in the GZ, zlog-mode=nologging
 *
...
function buildDockerLogZonecfg(vmobj, payload, log) {
    ...
    if (new_tty_value === true) {
        if (log_driver === 'json-file') {
            new_logmode = 'interactive';
        } else {
            new_logmode = 'nlinteractive';
        }
    } else { // new_tty_value === false
        if (log_driver === 'json-file') {
            new_logmode = 'logging';
        } else {
            new_logmode = 'nologging';
        }
    }
...
function setRctl(zonename, brand, rctl, value, log, callback)
{
    ...
    if (brand == 'solaris10') {
        args = args.concat(['process', '`/usr/bin/pgrep', '-oxz', zonename, 'init`']);
    } else {
        args = args.concat(['zone', zonename]);
    }
...
function removeRctl(zonename, brand, rctl, log, callback)
{
    var args;
...
function updateVnicAllowedIPs(uuid, nic, log, callback)
{
    ...
    if (nic.hasOwnProperty('ips')) {
        nic.ips.forEach(function (ip) {
            ...
            ips.push(ipaddr.splitCIDR(ip).ip);
        });
    }
            ...
            async.forEachSeries(keys, function (prop, c) {
                ...
                if (payload.hasOwnProperty(prop)) {
                    if (prop === 'cpu_cap' && Number(payload[prop]) === 0) {
                        ...
                        // setting to 0 fails.
                        removeRctl(newobj.zonename, newobj.brand, rctl, log, function (err) {
                            if (err) {
