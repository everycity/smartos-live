...
function loadTags(vmobj, log, callback)
{
    var filename;
        ...
        fs.readFile(filename, function (error, data) {
            ...
            if (error) {
                if (error.code === 'ENOENT') {
                    ...
                    log.error(error, 'loadTags() failed to load tags from '
                        + filename);
                    callback(error, {});
                    return;
                }
...
function loadMetadata(vmobj, log, callback)
{
    var filename;
        ...
        fs.readFile(filename, function (error, data) {
            ...
            if (error) {
                if (error.code === 'ENOENT') {
                    ...
                    log.error(error, 'loadMetadata() failed to load metadata '
                        + 'from ' + filename);
                    callback(error, {});
                }
            } else {
    ...
    getVmobj(info.zonename, data, options, function (err, vmobj) {
        ...
        async.series([
            function (cb) {
                ...
                loadMetadata(vmobj, log, function (error, metadata) {
                    if (error) {
                ...
                loadTags(vmobj, log, function (error, tags) {
                    if (error) {
...
exports.reprovision = function (uuid, payload, options, callback)
{
    ...
    async.waterfall([
        function (cb) {
            VM.load(uuid, {
                fields: [
                    'brand', 'datasets', 'hostname', 'nics', 'state', 'uuid',
                    'zfs_filesystem', 'zone_state', 'zonename', 'zonepath',
                    'zpool'
                ],
                log: log
            }, function (err, obj) {
            ...
            zfs(['rename', vmobj.datasets[0], vmobj.zfs_filesystem
            ...
            zfs(['rename', vmobj.zfs_filesystem, vmobj.zfs_filesystem
            ...
            var snapname = vmobj.zpool + '/' + payload.image_uuid + '@'
                + vmobj.uuid;
            ...
            zfs(['clone', vmobj.zpool + '/' + payload.image_uuid + '@'
                + vmobj.uuid, vmobj.zfs_filesystem], log, function (err, fds) {
            ...
            zfs(['destroy', vmobj.zfs_filesystem + '-reprovisioning-root'], log,
                function (err, fds) {
            ...
            zfs(['rename', vmobj.zfs_filesystem + '-reprovisioning-data',
...
exports.delete_snapshot = function (uuid, snapname, options, callback)
{
    ...
    VM.load(uuid, {fields: load_fields, log: log}, function (err, vmobj) {
        ...
        args = ['destroy', vmobj.zfs_filesystem + '@vmsnap-' + snapname];
        zfs(args, log, function (zfs_err, fds) {
            if (zfs_err) {
                log.error({'err': zfs_err, 'stdout': fds.stdout,
            ...
            log.debug({err: zfs_err, stdout: fds.stdout, stderr: fds.stderr},
                'zfs destroy ' + vmobj.zfs_filesystem + '@vmsnap-' + snapname);
...
function setRctl(zonename, brand, rctl, value, log, callback)
{
...
function firewallVMrun(opts, fn, log, callback)
{
    ...
    // fields list here comes from the list of fields used in generateVM() in
    // src/fw/test/lib/helpers.js as that's the test VM object the fw tests use.
    fw_fields = [
        'firewall_enabled',
        'nics',
        'owner_uuid',
    ...
    VM.lookup({}, {fields: fw_fields, log: log}, function (err, records) {
        if (err) {
            callback(err);
            return;
        }
