                ...
                async.forEachSeries(payload.update_disks, function (disk, c) {
                    ...
                    for (d in oldobj.disks) {
                        ...
                        if (d.path === disk.path
                            ...
                            if (!error && disk.hasOwnProperty('compression')) {
                                zfs(['set', 'compression=' + disk.compression,
                                    d.zfs_filesystem], log,
                                    function (err, fds) {

                                    if (err) {
                                        log.error(err, 'failed to set '
                                            + 'compression=' + disk.compression
                                            + ' for ' + d.zfs_filesystem);
                                        error = err;
                                    }
                                });
                            }
                            ...
                            if (!error
                                && disk.hasOwnProperty('refreservation')) {

                                zfs(['set', 'refreservation='
                                    + disk.refreservation + 'M',
                                    d.zfs_filesystem], log,
                                    function (err, fds) {
